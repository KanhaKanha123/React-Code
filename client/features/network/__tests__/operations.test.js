...
describe('Testing on network operations', () => {
  ...
  describe('Test on callApi', () => {
    test('Expect netFailOffline not called if network is online', () => {
      let callApiArg = {
        service: 'CashierService.GetTimeStamp',
        skipSessionCheck: true
      }
      let netFailOfflineSpy = jest.spyOn(networkOperations, 'netFailOffline')
      store.dispatch(networkOperations.callApi(callApiArg)) //.then(() => {
